...
const MESSAGES = {
  ko: {
    ...
    hlDone: () => '📋 *실행자 작업 완료 보고*',
    verificationPending: () => '독립 검증 대기 중입니다. GIIP REVIEW는 검증 완료를 뜻하지 않습니다.',
    taskEvidenceLabel: ({ state }) => `검증: ${{ prepared: '준비만 됨', executing: '실행 관측', executor_reported: '실행자 완료 보고, 검증 대기', failed: '실행 실패', verified: '독립 검증 통과', unknown: '증거 없음' }[state] || '증거 없음'}`,
    issueReviewReport: ({ result }) => `📋 실행자 작업 완료 보고. 독립 검증 대기(REVIEW). ${result}`,
  ...
  en: {
    ...
    hlDone: () => '📋 *Executor reports task complete*',
    verificationPending: () => 'Independent verification is pending. GIIP REVIEW does not mean verified.',
    taskEvidenceLabel: ({ state }) => `Verification: ${{ prepared: 'prepared only', executing: 'execution observed', executor_reported: 'executor report; review pending', failed: 'execution failed', verified: 'independently verified', unknown: 'no evidence' }[state] || 'no evidence'}`,
    issueReviewReport: ({ result }) => `📋 Executor reports task complete. Independent verification pending (REVIEW). ${result}`,
  ...
  ja: {
    ...
    hlDone: () => '📋 *実行者による作業完了報告*',
    verificationPending: () => '独立した検証待ちです。GIIP REVIEW は検証完了を意味しません。',
    taskEvidenceLabel: ({ state }) => `検証: ${{ prepared: '準備のみ', executing: '実行を確認', executor_reported: '実行者完了報告・検証待ち', failed: '実行失敗', verified: '独立検証済み', unknown: '証拠なし' }[state] || '証拠なし'}`,
    issueReviewReport: ({ result }) => `📋 実行者による作業完了報告。独立した検証待ち(REVIEW)。${result}`,
