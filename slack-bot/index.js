...
// ── タスクID抽出・類似判定・重複統合 (task-dedup.js に切り出し) ───────────────
const {
  extractExistingTaskIds, findSimilarPendingTasks, loadPendingTaskMeta,
  tasksAreSimilar, findDuplicatePendingClusters, applyTaskMerge,
} = require("./task-dedup");

// ── コマンド判定・意図分類 (intent.js に切り出し) ────────────────────────────
const { isGoCmd, isCancelCmd, isPureGitOp, classifyRequest } = require("./intent");

// ── claude CLI / spawn ヘルパー (claude-cli.js に切り出し) ─────────────────────
const { spawnAsync, isClaudeAuthed, reAuthClaude, callClaude, gitPullBeforeWork } = require("./claude-cli");
