...
#           (4) headless 엔진 시그니처(-p + --dangerously-skip-permissions + --add-dir)만 대상
#           (5) 부모가 살아있으면(=활성 세션/실행 중 잡) 나이와 무관하게 종료 금지 — 고아만 30분 기준 회수
if (-not $DryRun) {
    ...
    foreach ($cp in @(Get-Process claude -ErrorAction SilentlyContinue)) {
        try {
            ...
            if (-not ($cmd -match '(^|\s)(-p|--print)(\s|$)' -and $cmd -like '*--dangerously-skip-permissions*' -and $cmd -like '*--add-dir*')) {
                continue  # headless 엔진이 아닌 claude(대화형/remote-control 등)는 절대 비대상
            }
            $parentAlive = $false
