...
메신저를 통해 에이전트 시스템과 대화하려면 아래 항목들이 서버/로컬 환경에 준비되어야 합니다.
...
- **명령 예시**:
    ```text
    현재 작업 현황 알려줘 (현재 프로젝트 상태 및 태스크 확인)
    @에이전트 /pdca를 사용해서 오늘 작업한 내용을 요약해줘.
    ```

### 3. 정상 작동 확인 (Validation)
메신저에서 에이전트가 시스템의 규칙을 인식하고 있는지 확인하려면 다음을 입력하세요.
- **입력**: `/status`
- **기대 결과**: 에이전트가 메신저 채널을 통해 "Agent Orchestrator"로서 응답하며, 프로젝트 루트의 `.agent/` 정보를 바탕으로 현재 작업 현황을 브리핑해야 합니다.

### 4. 최신 버전 업데이트 (Update)
OpenClaw 환경에서 에이전트 시스템의 최신 규칙을 유지하려면 메신저에서 다음과 같이 요청하세요.

> **업데이트 프롬프트**:
> `사용 중인 에이전트 설정 저장소의 최신 .agent 폴더와 GEMINI.md 내용을 확인하고, 현재 연결된 워크스페이스의 에이전트 시스템을 최신 상태로 업데이트해줘.`
...
---
*본 에이전트 시스템은 OpenClaw를 통해 프로젝트의 지식을 메신저로 확장하여 진정한 '주머니 속의 개발팀'을 완성합니다.*
